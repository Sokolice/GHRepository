...
const HarvestComponent = observer(function Harvest({ plantDTO }: Props) {
  ...
  const posibleNextPlants = Array.from(plantStore.plantDTOList).filter(
    (item) => item[1].cropRotation != plantDTO?.cropRotation,
  );
      ...
      <Grid columns={2}>
        <GridRow>
          <GridColumn width={6}>
            <Image
              size="medium"
              src={`/src/assets/plants/${plantDTO?.imageSrc}`}
              wrapped
            />
          </GridColumn>
