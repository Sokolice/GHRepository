...
const NewPlantForm = observer(function NewPlantForm() {
  ...
  return (
    <>
      <Form
        onSubmit={() => plantStore.createNewPlant(initialValue, plantTypeId)}
      >
        <Header>Přidání nové rostliny</Header>
        <Select
          name="plantType"
          placeholder="Typ"
          options={allPLantTypesOptions}
          disabled={false}
          onChange={handlePlantTypeChangeClick}
        />
        <FormField
          name="name"
          placeholder="Název"
        ...
        <FormCheckbox
          name="directSewing"
          label="Přímý výsev"
          checked={initialValue.directSewing}
          disabled={disabled}
          onChange={handleCheckBoxChange}
        />
        <FormCheckbox
          name="preCultivation"
          label="Předpěstovíní"
          checked={initialValue.preCultivation}
          disabled={disabled}
          onChange={handleCheckBoxChange}
        />
        <FormInput
          ...
          type="number"
          label="Teplota klíčení"
          value={initialValue.germinationTemp}
          disabled={disabled}
          onChange={handleChange}
        />
        <FormTextArea
        ...
        <FormInput
          ...
          label="Opakovaný výsev po dnech"
          placeholder="Opakovaný výsev po dnech"
          value={initialValue.repeatedPlanting}
          disabled={disabled}
          onChange={handleChange}
        />
        <FormCheckbox
          name="differentPlantingMonths"
          label="Upravit data výsadby a sklizně"
          checked={differentPlanting}
          disabled={disabled}
          onChange={() => setDifferentPlanting(!differentPlanting)}
        />
        <Grid columns={2}>
          <GridRow>
            <GridColumn>
          ...
          <GridRow>
            <GridColumn>
              <MonthWeekSelect
                disabled={!differentPlanting}
                key="sowingFrom"
                value={plantStore.newPlantSowingFrom}
              />
            </GridColumn>
            <GridColumn>
              <MonthWeekSelect
                disabled={!differentPlanting}
                key="sowingTo"
                value={plantStore.newPlantSowingTo}
              />
            </GridColumn>
          </GridRow>
        </Grid>
        <Grid columns={2}>
          ...
          <GridRow>
            <GridColumn>
              <MonthWeekSelect
                disabled={!differentPlanting}
                value={plantStore.newPlantHarvestFrom}
              />
            </GridColumn>
            <GridColumn>
              <MonthWeekSelect
                disabled={!differentPlanting}
                value={plantStore.newPlantHarvestTo}
              />
            </GridColumn>
          </GridRow>
        </Grid>
