...
const BedComponent = observer(function Bed() {
  ...
  useEffect(() => {
    ...
    loadPlantDTO,
    plantDTOList.size,
    allPlantsRelations.length,
    loadAllPlantsRelations,
  ]);

  function pushToOptions(
    key: string,
    text: string,
    value: string,
    image: string,
    ...
    runInAction(() => {
      ...
      selectedBed?.cells.forEach((cell) => {
        ...
        if (cell.x == minColumn && cell.y == minRow) {
          cell.gridArea =
            minColumn + " / " + minRow + " / span " + maxC + " / span " + maxR;
  ...
  function renderCell(cell: Cell) {
              ...
              <Label
                ...
                to={
                  selectedBed?.bed.isDesign
                    ? `/plants/${id}/beds/${selectedBed.bed.id}`
                    : "/plantrecords"
                }
              >
                {plant?.name}{" "}
    ...
    return (
      <div
        className={"grid-item" + (cell.isActive ? " clicked" : "")}
              ...
              onClick={() =>
                store.modalStore.openModal(
                  <ConfirmationDeleteComponent
                    handleSubmit={() => deleteClick(cell.objectID)}
                  />,
                )
              }
              className="no_print, dell_Cell"
            />
            {!selectedBed.bed.isDesign ? (
              <Button
                size="tiny"
                className="no_print, harvest_Cell"
                onClick={() =>
                  store.modalStore.openModal(
  ...
  return (
    <SegmentGroup>
      ...
      <Segment>
        ...
        <div
          className="grid-container printable"
          id={`bed_${selectedBed.bed.id}`}
          key={`bed_${selectedBed.bed.id}`}
          style={generateStyle()}
        >
        ...
        <div className="hidden center">
          {"delka " +
            selectedBed.bed.length +
            " m ---- " +
            "šířka" +
            selectedBed.bed.width +
            " m"}
        </div>
      </Segment>
      <Segment>
