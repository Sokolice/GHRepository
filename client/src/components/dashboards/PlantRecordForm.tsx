...
const PlantRecordFormComponent = observer(function PlantRecordForm({
  plant,
  plantRecordId,
}: Props) {
  ...
  useEffect(() => {
    if (plantRecordId != "") {
      store.plantRecordStore
        .loadPlantRecord(plantRecordId!)
        .then((plantRecord) => setRecord(plantRecord!));
    }
  }, [plantRecordId]);
