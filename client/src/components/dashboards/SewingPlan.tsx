    ...
    <Card key={plant.id}>
      ...
      <Card.Content>
        <Card.Header>
          {plant.name} &nbsp;
          {toSowMonth &&
          store.globalStore.stats?.canBeSowedThisWeek.includes(plant.id) ? (
            <Popup
              content="Zbývá vysadit tento týden"
              trigger={
                <Label color="red">
      ...
      <Card.Content extra>
        {plant.directSewing ? (
          <Popup
            content="Přímý výsev"
            trigger={<Icon name="tree" size="large" color="green" />}
          />
        ) : (
          <Popup
            content="Předpěstování"
            trigger={<Icon name="home" size="large" color="green" />}
          />
        )}

        {plant.isHybrid ? (
          <Popup
            content="Hybridní odrůda"
            trigger={<Icon name="pagelines" size="large" color="brown" />}
          />
        ) : null}
        <Divider horizontal />
        <Button
          ...
          onClick={() =>
            store.modalStore.openModal(
              <PlantRecordFormComponent plant={plant} plantRecordId={""} />,
            )
          }
          content="Zasaď mě"
          key={plant.id}
...
const RenderMonthWeek = (monthSewed: MonthSewedRelation) => {
  ...
  return (
    <Container
      key={`${monthSewed.month}`}
      id={`month_${mapMonthIndex(monthSewed.month)}`}
      textAlign="center"
    >
      <Divider horizontal />

      <Label color="olive">
        <Header as="h2">{monthSewed.month}</Header>
      </Label>
      <Divider horizontal />
      <Card.Group itemsPerRow={6}>
        {monthSewed.sewedPlants.map((plant: PlantDTO) =>
          RenderPlant(plant, thisMonth == mapMonthIndex(monthSewed.month)),
        )}
      </Card.Group>
    </Container>
  );
...
const SewingPlanComponent = observer(function SewingPlan() {
  ...
  function switchToCurrentMonth() {
    ...
    store.monthWeekStore.isCurrentMonthActive =
      !store.monthWeekStore.isCurrentMonthActive;
  ...
  return (
    <Container>
      <Container textAlign="center">
        <Button toggle active={active} onClick={switchToCurrentMonth}>
          Aktuální výsev
        </Button>
        <Button toggle active={planted} onClick={hidePlanted}>
          Skrýt vyseté
        </Button>
        <Button as={Link} to="/myPlants">
          Moje rostliny
        </Button>
      </Container>

      <Container>
        {store.monthWeekStore.currentMonthRelationList.map(
          (m: MonthSewedRelation) => {
