...
const MyPlantsComponent = observer(function MyPlants() {
  ...
  const {
    plantDTOList,
    allAvailablePlantDTOListGrouped,
    loadAllPlants,
    allPlantTypes,
    loadAllPlantTypes,
  } = plantStore;
  ...
  return (
    <>
      <Button
        icon="plus"
        onClick={() => store.modalStore.openModal(<NewPlantForm />)}
        content="Nova rostlina"
      />
      <Button
        positive
        onClick={() => store.plantStore.savePlantsForUser(plantsToAdd)}
        content="Ulozit"
      />
      <Divider hidden horizontal />
      <Grid columns={2}>
        <GridColumn>
          <Header>Moje rostliny</Header>
          {plantDTOList.size > 0 ? (
            <List>
              {Array.from(plantDTOList).map((plant) => {
                return (
                  <ListItem key={"p_" + plant[0]}>
                    <Image
                      avatar
                      src={`/src/assets/plants/${plant[1].imageSrc}`}
                    />
                    <ListContent>
                      <ListHeader>{plant[1].name}</ListHeader>
                    </ListContent>
                  </ListItem>
                );
              })}
            ...
            <label>
              Seznam rostlin je prazdny, pridej z vyberu, nebo vytvor nove
            </label>
          )}
        </GridColumn>
        <GridColumn>
              ...
              return (
                <>
                  ...
                  >
                    ...
                    <Image
                      avatar
                      src={`/src/assets/plants/${group.plantType.imageSrc}`}
                    />
