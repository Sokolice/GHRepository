...
export default class MonthWeekStore {
  ...
  filterMonthWeeks = () => {
    if (this.isCurrentMonthActive) {
      ...
      const filteredMonthWeeks = this.currentMonthRelationList.filter(
        (item) => mapMonthIndex(item.month) == month,
      );
  ...
  filterPlanted = () => {
    if (this.hidePlanted) {
      const plantedIds = store.plantRecordStore.plantRecords.map(
        (item) => item.plantId,
      );
      ...
      this.currentMonthRelationList.forEach((relation) => {
        newArr.push({
          ...
          sewedPlants: relation.sewedPlants.filter(
            (plant) => !plantedIds.includes(plant.id),
          ),
        });
      });
