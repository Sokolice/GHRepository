...
namespace API_Recipes
{
    public class ApetiteParser
    {
        public static List<Recipe> GetRecipesByIngredient(string aIngredient)
        {
            ...
            {
               var name = liElement.Descendants("a").Where(a=> a.HasClass("a-teaser__link")).FirstOrDefault().Attributes["title"].Value;
               var urlLi = liElement.Descendants("a").Where(a => a.HasClass("a-teaser__link")).FirstOrDefault().Attributes["href"].Value;
